...
template <class T, int MAXSIZE = 500000,
          class alloc = myAllocator::array<T, MAXSIZE + 2>>
struct tree {
  alloc pool;
  T *root;
...
splay::tree<node, MAXSIZE, myAllocator::stack<node, MAXSIZE + 2>> T;
...
int main(){
	...
	while(t--){
		...
		cin >> s;
		for (char c : s){
			T.insert(T.new_node(node(c)), T.size());
		}
		vector<int> l(k), r(k);
		for (auto &it : l){
			cin >> it;
		...
		for (int i = 0; i < q; i++){
			int x;
			cin >> x;
			auto it = upper_bound(l.begin(), l.end(), x) - l.begin() - 1;
			int lb = l[it], up = r[it];
			assert(lb <= x && x <= up);
			int from = min(x, lb + up - x), to = max(x, lb + up - x);
			from--;
			T.find_range(from, to)->reverse();
		}
		for (int i = 0; i < n; i++){
			cout << (char)T.find_range(i,i+1)->val;
		}
		cout << '\n';
		T.erase_range(0, T.size());
	}
	return 0;
}
